...
public class Board {
	...
	private void getNumPlayers() {
		...
		try {
			...
			if (this.numPlayers < 2 || this.numPlayers > LIMIT)
		...
		try {
						...
						while (true) {
							...
							if (act.getId() == Actions.GET_NAME) {
								...
								if (act.getFrom() == null || act.getFrom().isEmpty()) {
									...
									/*
									 * Envia mensagem de aguardando demais
									 * jogadores.
									 */
									act = new Actions();
			...
			for (Iterator<String> iterator = players.keySet().iterator(); players
				...
				if (actions.getId() == Actions.INCOME) {
					coup.play(Actions.INCOME, actions.getFrom(), actions.getTo(), actions.isContest(),
							actions.isBlock());
					...
					/* Atualiza o jogo de todos os players. */
					String msg = "O jogador " + actions.getFrom() + " deu um golpe de estado no jogador "
							+ actions.getTo() + ".";
					actions.setId(Actions.UPDATE_ALL_INTERFACE);
					actions.setCoup(coup);
					actions.setLog(msg);
